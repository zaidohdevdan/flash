    ...
    return (
        <div className="flex flex-col h-full animate-in fade-in duration-500">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
                <div>
                    <h1 className="text-3xl font-black text-slate-900 dark:text-white tracking-tighter uppercase">Arquivo Base</h1>
                    <p className="text-slate-500 dark:text-slate-400 font-bold mt-2 uppercase tracking-[0.2em] text-[12px]">Histórico de Demandas Resolvidas</p>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-4">
                    ...
                    <div className="flex items-center bg-white dark:bg-black/40 border border-slate-200 dark:border-white/5 p-1 rounded-2xl shadow-sm backdrop-blur-md">
                        <div className="flex flex-wrap items-center bg-transparent border-0">
                            <div className="flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2 border-r border-slate-200 dark:border-white/5">
                                <Calendar className="w-4 h-4 text-slate-500 hidden sm:block" />
                                <div className="flex flex-col">
                                    <span className="text-[10px] sm:text-[11px] font-black text-slate-400 uppercase leading-none mb-1">Início</span>
                                    <input
                                        type="date"
                                        title="Data Início"
                                        value={startDate}
                                        onChange={e => { setStartDate(e.target.value); setPage(1); }}
                                        className="bg-transparent text-[11px] sm:text-[13px] font-black outline-none text-slate-700 dark:text-slate-300 uppercase cursor-pointer p-0 m-0 w-[100px] sm:w-[110px]"
                                    />
                                </div>
                            </div>
                            <div className="flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2">
                                <div className="flex flex-col">
                                    <span className="text-[10px] sm:text-[11px] font-black text-slate-400 uppercase leading-none mb-1">Término</span>
                                    <input
                                        type="date"
                                        title="Data Término"
                                        value={endDate}
