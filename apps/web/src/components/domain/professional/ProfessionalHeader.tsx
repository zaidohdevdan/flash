    ...
    return (
        <header className="flex justify-between items-center mb-8">
            <div>
                ...
                <p className="text-[10px] text-[var(--text-tertiary)] font-black uppercase tracking-widest mt-1">Seu histórico operacional</p>
            </div>
            ...
            <div className="flex items-center gap-1.5 px-3 py-1.5 bg-white border border-[var(--border-subtle)] rounded-full shadow-sm">
                ...
                <span className={`text-[9px] font-black uppercase ${isConnected ? 'text-emerald-600' : 'text-red-500'}`}>{isConnected ? 'Online' : 'Offline'}</span>
            </div>
        </header>
    );
};
